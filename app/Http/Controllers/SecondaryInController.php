...
class SecondaryInController extends Controller
{
    ...
    public function getSecondaryInList(Request $request)
    {
        ...
        $secondaryInSize = "";
        if ($request->secondaryInSize) {
            $secondaryInSize = "
                AND output_rfts.so_det_id = '".$request->secondaryInSelectedSize."'
            ";
        }

        $secondaryInSecondary = "";
        if ($request->selectedSecondary) {
            $secondaryInSecondary = "
                AND output_secondary_master.id = '".$request->selectedSecondary."'
            ";
        }

        $secondaryInMasterPlan = "";
        if ($request->secondaryInSelectedMasterPlan) {
            $secondaryInMasterPlan = "
                AND master_plan.id = '".$request->secondaryInSelectedMasterPlan."'
            ";
        }
        ...
        $secondaryInList = collect(
            DB::select("
                ...
                WHERE
                    ...
                    AND output_rfts.updated_at >= '2025-12-01 00:00:00'
                    AND output_secondary_master.id = '".$request->selectedSecondary."'
                    ".$secondaryInSearch."
                    ".$secondaryInFilterKode."
                    ".$secondaryInFilterLine."
