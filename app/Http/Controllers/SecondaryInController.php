...
class SecondaryInController extends Controller
{
    ...
    public function exportSecondaryInOut(Request $request) {
        ...
        $dateFrom = $request->dateFrom ? $request->dateFrom : date('Y-m-d');
        $dateTo = $request->dateTo ? $request->dateTo : date('Y-m-d');
        $selectedSecondary = $request->selectedSecondary ? $request->selectedSecondary : '';

        $data = SewingSecondaryIn::selectRaw("
                output_secondary_in.updated_at time_in,
                output_secondary_out.updated_at time_out,
                userpassword.username sewing_line,
                output_secondary_in.kode_numbering,
            ...
            whereRaw("
                (
                    output_secondary_in.updated_at between '".$dateFrom." 00:00:00' and '".$dateTo." 23:59:59'
                )
            ")->
            whereRaw("
                (
                    output_secondary_in.id IS NOT NULL AND
                    output_rfts.id IS NOT NULL AND
                    output_secondary_master.id = '".$selectedSecondary."'
                )
            ")->
            groupBy("output_secondary_in.id")->
            get();
        ...
        // Headers
        $sheet->writeTo('A1', 'Time IN')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('B1', 'Time OUT')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('C1', 'QR')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('D1', 'Line')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('E1', 'No. WS')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('F1', 'Style')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('G1', 'Color')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('H1', 'Size')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('I1', 'Status')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('J1', 'Secondary')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('K1', 'Type')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('L1', 'Area')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('M1', 'IN By')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        $sheet->writeTo('N1', 'OUT By')->applyFontStyleBold()->applyBorder(\PhpOffice\PhpSpreadsheet\Style\Border::BORDER_THIN);
        ...
        $filename = 'Secondary In Out ' . $dateFrom . ' - ' . $dateTo . ' (' . Carbon::now()->format('Y-m-d H:i:s') . ').xlsx';
